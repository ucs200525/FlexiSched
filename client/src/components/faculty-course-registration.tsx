...
import { Search, BookOpen, Plus, Trash2, Clock, GraduationCap, AlertCircle, Users } from "lucide-react";
import { apiRequest, queryClient } from "@/lib/queryClient";
import { useToast } from "@/hooks/use-toast";
import { useAuth } from "@/contexts/AuthContext";
import type { Course } from "@shared/schema";

export default function FacultyCourseRegistration() {
  const { toast } = useToast();
  const { user, isAuthenticated, isLoading: authLoading } = useAuth();
  const [searchTerm, setSearchTerm] = useState("");
  
  // Fetch available courses for the faculty
  const { data: availableCourses, isLoading: loadingAvailable, error: availableCoursesError } = useQuery<Course[]>({
    queryKey: [`/api/faculty/${user?.id}/available-courses`],
  ...
  const { data: assignedCourses, isLoading: loadingAssigned, error: assignedCoursesError } = useQuery<Course[]>({
    queryKey: [`/api/faculty/${user?.id}/assigned-courses`],
    enabled: !!user?.id && isAuthenticated && !authLoading,
    retry: 1,
    retryDelay: 1000,
  });
  ...
  const assignCourseMutation = useMutation({
    mutationFn: async (courseId: string) => {
      const response = await apiRequest("POST", `/api/faculty/${user?.id}/assign-course`, {
        courseId
      });
    ...
    onSuccess: (data) => {
      toast({
        title: "Success",
        description: "Successfully assigned to course!",
      });
      // Invalidate both available and assigned courses
      queryClient.invalidateQueries({ queryKey: [`/api/faculty/${user?.id}/available-courses`] });
      queryClient.invalidateQueries({ queryKey: [`/api/faculty/${user?.id}/assigned-courses`] });
    },
    onError: (error: any) => {
      let title = "Assignment Failed";
      let description = "Failed to assign course";
      
      if (error?.status === 401 || error?.message?.includes("Unauthorized")) {
        title = "Authentication Required";
        description = "Please log in to assign courses";
  ...
  // Course unassignment mutation
  const unassignCourseMutation = useMutation({
    mutationFn: async (courseId: string) => {
      const response = await apiRequest("DELETE", `/api/faculty/${user?.id}/assign-course/${courseId}`);
      return response.json();
    },
    onSuccess: (data) => {
      toast({
        title: "Success", 
        description: "Successfully unassigned from course!",
      });
      // Invalidate both available and assigned courses
      queryClient.invalidateQueries({ queryKey: [`/api/faculty/${user?.id}/available-courses`] });
      queryClient.invalidateQueries({ queryKey: [`/api/faculty/${user?.id}/assigned-courses`] });
    ...
    onError: (error: any) => {
      let title = "Unassignment Failed";
      let description = "Failed to unassign from course";
      
      if (error?.status === 401 || error?.message?.includes("Unauthorized")) {
        title = "Authentication Required";
        description = "Please log in to unassign courses";
      } else if (error instanceof Error) {
        description = error.message;
      }
  ...
  const handleUnassignCourse = (courseId: string) => {
    ...
    unassignCourseMutation.mutate(courseId);
  };
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground text-center mb-4">
          Please log in to access course assignment
        </p>
      </div>
    );
  }
    ...
    <div className="flex-1 flex flex-col">
      ...
      <header className="bg-card border-b border-border px-6 py-4">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold text-foreground" data-testid="page-title">Course Assignment</h1>
            <p className="text-sm text-muted-foreground">
              Assign courses to teach and manage your teaching load
            </p>
          </div>
        </div>
      </header>
      ...
      <main className="flex-1 p-6 overflow-auto">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-2xl font-bold text-foreground" data-testid="stat-assigned-courses">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div className="space-y-2">
                  ...
                  <p className="text-2xl font-bold text-foreground" data-testid="stat-teaching-hours">
                    {assignedCourses?.reduce((sum, course) => sum + (course.theoryHours + course.practicalHours), 0) || 0}
                  </p>
        ...
        <Tabs defaultValue="available" className="space-y-6">
          <TabsList className="grid w-full grid-cols-2">
            ...
            <TabsTrigger value="assigned" data-testid="tab-assigned-courses">
                        ...
                        <TableRow key={course.id} data-testid={`available-course-${course.id}`}>
                          ...
                          <TableCell>
                            <div className="text-sm">
                              ...
                              <div className="text-muted-foreground">Sem {course.semester}</div>
                            </div>
                          </TableCell>
                          <TableCell>
                            <Button
            ...
            <Card>
              ...
              <CardContent>
                {loadingAssigned ? (
                  <div className="text-center py-8">
                    <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
                    <p className="mt-2 text-sm text-muted-foreground">Loading assigned courses...</p>
                  </div>
                ) : assignedCoursesError ? (
                  <div className="text-center py-8">
                    <AlertCircle className="w-12 h-12 text-destructive mx-auto mb-4" />
                    <p className="text-destructive font-medium mb-2">Failed to Load Assigned Courses</p>
                    <p className="text-sm text-muted-foreground">
                      {(assignedCoursesError as any)?.status === 401 
                        ? "Please log in to view your assigned courses" 
                        : "Please try refreshing the page"}
                    </p>
                  </div>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredAssignedCourses.map((course) => (
                        <TableRow key={course.id} data-testid={`assigned-course-${course.id}`}>
                          <TableCell className="font-medium">{course.courseCode}</TableCell>
                          <TableCell>
                            <div>
                              <p className="font-medium">{course.courseName}</p>
                          ...
                          <TableCell>
                            <Button
                              ...
                              data-testid={`button-unassign-${course.id}`}
                            >
